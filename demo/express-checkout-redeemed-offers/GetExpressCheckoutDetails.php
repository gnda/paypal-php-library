...
if($PayPal->APICallSuccessful($PayPalResult['ACK']))
{
    ...
    /* This code is to get details of wallet and instruments of the buyer */
    $n=0;
    if(isset($PayPalResult['WALLETTYPE'.$n]) || isset($PayPalResult['WALLETID'.$n]) || isset($PayPalResult['WALLETDESCRIPTION'.$n])){
        while(isset($PayPalResult['WALLETTYPE'.$n])){
            $_SESSION['wallet_type_'.$n] = isset($PayPalResult['WALLETTYPE'.$n]) ? $PayPalResult['WALLETTYPE'.$n] : '';
            $_SESSION['wallet_id_'.$n]   = isset($PayPalResult['WALLETID'.$n]) ? $PayPalResult['WALLETID'.$n] : '';
            $_SESSION['wallet_desc_'.$n] = isset($PayPalResult['WALLETDESCRIPTION'.$n]) ? $PayPalResult['WALLETDESCRIPTION'.$n] : '';
            $n++;
        }
    }
    if(isset($PayPalResult['INSTRUMENTID'])){
        $_SESSION['instrument_category'] = isset($PayPalResult['INSTRUMENTCATEGORY']) ? $PayPalResult['INSTRUMENTCATEGORY'] : '';
        $_SESSION['instrument_id'] = isset($PayPalResult['INSTRUMENTID']) ? $PayPalResult['INSTRUMENTID'] : '';   
    }    
    /* END */
